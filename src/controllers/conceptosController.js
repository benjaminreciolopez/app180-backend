...
export async function listConceptos(req, res) {
    try {
        ...
        if (q) {
            const pattern = `%${q}%`;
            query = sql`
        ${query} and (nombre ilike ${pattern} or descripcion ilike ${pattern})
      `;
        }
...
export async function autocompleteConceptos(req, res) {
    try {
      ...
      where empresa_id=${empresaId}
        and (nombre ilike ${pattern} or descripcion ilike ${pattern})
      order by nombre asc
      limit 20
    `;
...
export async function createConcepto(req, res) {
    try {
      ...
      insert into concepto_180 (empresa_id, nombre, descripcion, precio_unitario, created_at)
      values (${empresaId}, ${nombre}, ${descripcion}, ${precio_unitario || 0}, now())
      returning *
    `;
